...
A shared glossary of motion patterns, adapted from [animations.dev/vocabulary](https://animations.dev/vocabulary) by Emil Kowalski. These are shared names for describing what to build, so a request maps to a known pattern instead of a vague gesture. For DEPTH on easing, spring physics, and performance — when to reach for each and how to tune it — go back to the decision tables in [../SKILL.md](../SKILL.md); this file names patterns rather than restating that guidance.
