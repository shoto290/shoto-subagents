...
| [`design-engineer`](./plugins/design-engineer) | 1 agent · 7 skills | Front-end specialist (React + TypeScript) for building the design system itself — tokens, primitive components, variants, theming, and Storybook. Sits downstream of designer. |
| [`frontend-engineer`](./plugins/frontend-engineer) | 1 agent · 9 skills | Front-end specialist (React + TypeScript) for building the applications that consume the design system — screens, routing, data fetching, state, forms, and app performance. Sits downstream of design-engineer. |
...
| [`engineering`](./plugins/engineering) | 8 skills | Senior-developer craft skills (senior-mindset, clean-code, SOLID/DRY/KISS/YAGNI, reuse-first, avoid-over-engineering, pragmatic-principles, scalable-architecture, secure-by-default) that every specialist agent preloads by default. |
...
Today: `product-designer` and `designer` are purple, `design-engineer`, `frontend-engineer` and `backend-engineer` are green. Agents sharing a color is expected — same action, different domain.
